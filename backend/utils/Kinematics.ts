...
export async function forwardKinematics (dhParams: any, jointAngles: any) {
    if (dhParams.length !== jointAngles.length) {
        throw new Error("Number of DH parameters must match number of joint angles.");
    }
...
export async function formatMatrix(matrix: Matrix) {
    ...
    const eulerAngles = extractEulerAngles(k);

}


function extractEulerAngles(matrix: Matrix | number[][]): { roll: number, pitch: number, yaw: number } {
    let data: number[][];
...
function rotationMatrixToEulerZYX(R: number[][]): { roll: number, pitch: number, yaw: number } {
